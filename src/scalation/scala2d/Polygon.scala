     ...
     extends java.awt.geom.Path2D.Double with Error
{
    ...
    /*:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
     * FIX: use ** once scaladoc is fixed, currently crashes.
 ...
 */
case class Octogon (vertex: Array [R2])
     extends java.awt.geom.Path2D.Double with Error
{
    {
        val n = vertex.length
        if (n != 8) flaw ("constructor", "need exactly 8 vertices to make a octogon")
        moveTo (vertex(0).x, vertex(0).y)
        for (i <- 1 until n) lineTo (vertex(i).x, vertex(i).y)
        lineTo (vertex(0).x, vertex(0).y)
    } // primary constructor
...
object PolygonTest extends Application
{
    ...
    private val octogon    = new Octogon  (R2 (400, 600), 70)
    private val octogonXY  = octogon.getTopLeft
    ...
    private val canvas = new Panel
    {
        ...
        override def paintComponent (g2d: Graphics2D)
        {
            ...
            g2d.setPaint (magenta); g2d.fill (octogon)
            g2d.setPaint (black);   dot.setFrame (octogonXY.x, octogonXY.y, 5, 5); g2d.fill (dot)
        } // paintComponent
