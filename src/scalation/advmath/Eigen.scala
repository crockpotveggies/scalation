 ...
 */
class Householder (a: SparseMatrixN [Double])
      extends Error
{
      ...
      extends Error
{
...

    {
        ...
        var dv = VectorN [Double] (t.dim1)
        var ev = VectorN [Double] (t.dim1 - 1)
...
object SymmetricQRstep extends Error
{
    private val EPSILON = 1e-9
 ...
 */
class EigenvalueSym (a: SparseMatrixN [Double])
      extends Error
{
    /** Flag indicating whether tracing is on to monitor convergence
     */
...
{
    val t = new MatrixD (4, 1., 1., 0.,   0.,                   // 4-by-4 matrix
                            1., 2., 1.,   0.,
                            0., 1., 3.,   0.01,
                            0., 0., 0.01, 4.)
